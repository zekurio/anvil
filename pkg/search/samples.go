...
func copySampleArgs(plan domain.EncodePlan, window sampleWindow, output string) []string {
	...
	return []string{"-hide_banner", "-nostdin", "-n", "-xerror", "-ss", seconds(window.offset), "-i", plan.InputPath,
...
func prepareSamples(ctx context.Context, runner process.Runner, plan domain.EncodePlan, windows []sampleWindow, dir string) ([]sample, error) {
	...
	for i, window := range windows {
		...
		if _, err := runner.Run(ctx, process.Command{Name: "ffmpeg", Args: copySampleArgs(plan, window, path)}); err != nil {
			return nil, fmt.Errorf("extract search sample %d at %.3fs: %w", i+1, window.offset, err)
		}
		frames, err := sampleFrames(ctx, runner, path, plan.Threads)
		if err != nil {
...
func measureCandidate(ctx context.Context, runner process.Runner, plan domain.EncodePlan, samples []sample, dir string, crf int) (domain.SearchCandidate, error) {
	candidate := domain.SearchCandidate{CRF: crf}
	...
	for i, ref := range samples {
		...
		if _, err := runner.Run(ctx, process.Command{Name: "ffmpeg", Args: ffmpeg.SampleArgs(samplePlan)}); err != nil {
			return candidate, fmt.Errorf("encode sample %d at CRF %d: %w", i+1, crf, err)
		}
		frames, err := sampleFrames(ctx, runner, encoded, plan.Threads)
		if err != nil {
		...
		score, err := measureQuality(ctx, runner, plan, ref, encoded, dir)
		if err != nil {
...
func sampleFrames(ctx context.Context, runner process.Runner, path string, threads int) (int, error) {
	result, err := runner.Run(ctx, process.Command{Name: "ffprobe", Args: sampleFrameArgs(path, threads), RequireFullStdout: true})
	if err != nil {
		return 0, err
	}
	// ffprobe may log decode errors while exiting successfully.
	if len(strings.TrimSpace(string(result.Stderr))) > 0 {
		return 0, fmt.Errorf("sample decode failed: %s", strings.TrimSpace(string(result.Stderr)))
	}
	var data struct {
		Streams []struct {
