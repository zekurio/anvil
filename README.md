...
Profiles can require AV1 encodes to save space during `ab-av1 crf-search` with `profiles.<name>.video.min_savings_percent`. Anvil maps this to `ab-av1 --max-encoded-percent`, so `min_savings_percent = 20` requires the fitted encode to be no larger than 80% of the input. If ab-av1 cannot find a CRF that satisfies the configured VMAF and savings policy, Anvil treats that as a non-fatal video-copy/remux path by default: audio, subtitle, metadata, attachment, chapter, validation, replacement, and handoff steps still run, but no AV1 CRF encode is applied. Set `profiles.<name>.video.force_encode_on_no_fit = true` to force an encode instead; when ab-av1 reports no suitable CRF, Anvil uses the lowest tested CRF from the search output, falling back to `crf_min` if the no-fit output does not include a CRF sample.
