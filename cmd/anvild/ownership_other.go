...
// daemonOwnership has no exclusive-lock implementation outside unix. Anvil is
// Linux-first, so rather than failing to build, the guard degrades to a loud
// warning: the control socket still refuses a second daemon once it starts
// listening, which happens before any start-up side effect.
